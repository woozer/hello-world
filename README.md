...
Onder **Tests** bij de pipeline en in de MR-testsamenvatting staan de JUnit-resultaten. Dependency-Check plaatst ook een MR-reactie met aantallen en bevindingen. Na analyse van main plaatst de centrale Sonar-helper de quality gate, meetwaarden en dashboardlink bij de commit en de bijbehorende gemergede MR. De volledige rapporten blijven beschikbaar als jobartifacts.

[.gitlab-ci.yml](.gitlab-ci.yml) neemt het gedeelde formulier en één centrale [java-service](https://github.com/woozer/ci-pipelines) op. CI-scripts en jobvolgorde staan in die bibliotheek. Helm leest eerst `environment/cluster/<cluster>.yaml` en daarna `environment/user/<profile>.yaml`. Het profiel `two-replicas` laat een aanpassing zien. Toegangsgegevens blijven in GitLab.

Formulier en pipeline gebruiken beide versie `1.1.0` van `ci-pipelines`. Die standaardpipeline zet de bouwblokken vast op `ci-components@2.0.0`. Werk bij een pipeline-upgrade beide verwijzingen in `.gitlab-ci.yml` samen bij via een merge request; zie de [versieafspraken](https://github.com/woozer/ci-components/blob/main/docs/component-versions.md).
